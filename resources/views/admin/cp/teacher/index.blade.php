...
<div class="card">
  <div class="card-body">
    <h4 class="card-title">المدرسيين المتاحيين</h4>
    <div class="row">
      <div class="col-12">
            ...
            @foreach ($teachers as $teacher)
              <tr>
                  ...
                  @if ($lang == 'ar')
                    ...
                    <td class="text-center">{{ $teacher->school->ar_name }}</td>
                    <td class="text-center">{{ $teacher->school->ar_delegate }}</td>
                  @elseif($lang == 'en')
                    <td class="text-center">{{ $teacher->en_name }}</td>
                    <td class="text-center">{{ $teacher->school->en_name }}</td>
                    <td class="text-center">{{ $teacher->school->en_delegate }}</td>
                  @endif
                  
                  
                  <td class="text-center">
                      <a class="btn btn-primary" href="{{ $path }}-students/{{ $teacher->id }}" target="_blank">
                      <i class="icon-user"></i>
                      {{ trans('main.students') }} ({{ $teacher->count_students }})
                      </a>
                  </td>
                  <td class="text-center"><a class="btn btn-primary" href="{{ $path }}/{{ $teacher->id }}" target="_blank">{{ trans('error.show') }}</a> | <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#deleteTeacher-{{ $teacher->id }}" >{{ trans('error.delete') }} </button></td>
              </tr>
              <div class="modal fade" id="deleteTeacher-{{ $teacher->id }}" tabindex="-1" role="dialog" aria-labelledby="deleteTeacher-4{{ $teacher->id }}" aria-hidden="true">
                  ...
                  <div class="modal-content">
                      ...
                      <form action="/admin/delete-teacher/{{ $teacher->id }}" method="POST">
