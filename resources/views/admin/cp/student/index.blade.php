...
<div class="card">
            <div class="card-body">
              <h4 class="card-title">طلاب المدارس</h4>
              <div class="row">
                <div class="col-12">
                    ...
                    <table class="table order-listing">
                      <thead>
                        <tr>
                            ...
                            <th class="text-center">{{ trans('indexing.school') }} </th>
                      ...
                      @foreach ($students as $student)
                        <tr>
                          @if ($lang == 'ar')
                            ...
                            <td class="text-center">{{ $student->school->ar_name }}</td>
                            <td class="text-center">{{ $student->teacher->ar_name }}</td>
                          @elseif($lang == 'en')
                            <td class="text-center">{{ $student->en_name }}</td>
                            <td class="text-center">{{ $student->school->en_name }}</td>
                            <td class="text-center">{{ $student->teacher->en_name }}</td>
                          @endif
                            <td class="text-center"><a class="btn btn-success" href="/{{ $lang }}{{ $path }}/report/{{ $student->id }}">{{ trans('error.report') }}</a> 
                            | <a class="btn btn-primary" href="{{ $path }}/{{ $student->id }}" target="_blank">{{ trans('error.show') }}</a> | <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#deleteStudent-{{ $student->id }}" >{{ trans('error.delete') }} </button></td>
                        </tr>
